...
#获得内容图片的名字
content_image_name = os.path.basename(settings.CONTENT_IMAGE_PATH)
content_image_name = content_image_name.split('.')[0]
#获得风格图片的名字
style_image_name = os.path.basename(settings.STYLE_IMAGE_PATH)
...
for epoch in range(settings.EPOCHS):
    ...
    with tqdm(total=settings.STEPS_PER_EPOCH, desc='Epoch {}/{}'.format(epoch + 1, settings.EPOCHS)) as pbar:
        ...
        for step in range(settings.STEPS_PER_EPOCH):
            ...
            pbar.update(1)
        # 每个epoch保存一次图片
        utils.save_image(noise_image, '{}/{}_{}.jpg'.format(settings.OUTPUT_DIR, content_image_name, style_image_name + str(epoch + 1)))
